    ...
    public function validasi(Request $request)
    {
        ...
        else
        {
            ...
            foreach ($databarangkeluarrumahsakit as $barisbkrs)
            {
                ...
                $databarangmasukrumahsakit = BarangMasukRumahSakitModel::where('id_urs', $id_urs)
                    ->where('kd_brg', $kd_brg)
                    ->where('sisa_bmrs', '!=', 0)
                ...
                foreach ($databarangmasukrumahsakit as $barisbmrs)
                {
                    ...
                    if ($jumlah_keluar <= $sisabmrs)
                    {
                        ...
                        BarangMasukRumahSakitModel::where('id_bmrs', $barisbmrs->id_bmrs)
                            ->update(['sisa_bmrs' => $sisa]);
                    ...
                    else
                    {
                        ...
                        if ($sisa >= 0)
                        {
                            ...
                            $barangCache[$kd_brg]->nilai_brg -= $nilai_baru;
                            $barangCache[$kd_brg]->stok_brg  -= $sisabmrs;
                ...
                DB::table('barang')->where('kd_brg', $kd_brg)->update([
                    'stok_brg'  => $stoktotal - $barisbkrs->jmlh_bkrs,
                    'nilai_brg' => $barangCache[$kd_brg]->nilai_brg,
                ]);
                $barangCache[$kd_brg]->stok_brg = $stoktotal - $barisbkrs->jmlh_bkrs;
            }
