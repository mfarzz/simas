...
class PbfkaCt extends Controller
{
    public function index()
    {
        ...
        return view('PermintaanBarang.KasiLogistik.Fakultas.index');
    }
    ...
    {  
        ...
        $tgl = date("Y-m-d");
        $user_id = auth()->user()->id;
        $cekDataStatus = PbsModel::where('untuk_pbs', 1)->where('level_pbs', 1)->where('urutan_pbs', 2)->first();
    ...
    public function validasi(Request $request)
    {
        ...
        $tgl = date("Y-m-d");
        $user_id = auth()->user()->id;
        $cekDataStatus = PbsModel::where('untuk_pbs', 1)->where('level_pbs', 3)->where('urutan_pbs', 1)->first();
