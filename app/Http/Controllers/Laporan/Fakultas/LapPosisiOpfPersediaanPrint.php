...
class LapPosisiOpfPersediaanPrint extends Controller
{
    Public Function index($filter)
    {
        ...
        {            
            ...
            $tjmlh_bmfd = 0;
            $databarangmasuk = BarangMasukFakultasDetailModel::
            where('id_bmf', '=', $barisbmf->id_bmf)
            ->where('tglperolehan_bmfd', '<=', $tgl_akhir )
            ->get();
            foreach($databarangmasuk as $barisbmfd)                    
            {
                $tjmlh_bmfd = $barisbmfd->jmlh_bmfd + $tjmlh_bmfd;
            }

            $tjmlh_opsik = 0;
            $databarangkeluaropsik = BarangKeluarFakultasDetailModel::
            join('opsik_fakultas','opsik_fakultas.id_opfk','=','barang_keluar_fakultas_detail.id_opfk')
            ->where('id_bmf', '=', $barisbmf->id_bmf)
            ->where('tgl_opfk', '<=', $tgl_akhir )
            ->get();
            foreach($databarangkeluaropsik as $barisbkfdopsik)                    
            {
                $tjmlh_opsik = $barisbkfdopsik->jmlh_bkfd + $tjmlh_opsik;
            }

            
            
            $sisa_tbmf = ($jmlh_awal_bmf + $tjmlh_bmfd) - ($tjmlh_bkfd + $tjmlh_opsik);
