...
class DaftarBarangRuanganFakultasCt extends Controller
{
    public function index()
    {
        ...
        if(request()->ajax()) {
            return datatables()->of(AsetDaftarRuanganFakultasModel::
            ...
            ->get())
            ->addColumn('a_id_adrf_en', function ($data) {
                $a_id_adrf_en = Crypt::encryptString($data->a_id_adrf);
                return $a_id_adrf_en; 
            })
            ->rawColumns(['action'])
            ->addIndexColumn()
        ...
        $daftar_ruangan = AsetRuanganFakultasModel::where('id_fk', $datafakultas->id_fk)->orderby('a_nm_arf')->get();
        return view('Sis_aset.Penggunaan.DaftarRuangan.Fakultas.index',['daftar_kategori'=> $daftar_kategori, 'daftar_ruangan'=> $daftar_ruangan]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $jumlah_barang = AsetDaftarRuanganFakultasDetailModel::join("aset_daftar_ruangan_fakultas",'aset_daftar_ruangan_fakultas_detail.a_id_adrf','=', 'aset_daftar_ruangan_fakultas.a_id_adrf')
        ...
        if($jumlah_barang == 0)
        {
            $datadaftar = new AsetDaftarRuanganFakultasModel();
            $datadaftar->a_kd_brg = $request->idBarang;
            $datadaftar->a_id_arf = $request->idRuangan;
            $datadaftar->a_jmlh_arf = $a_jmlh_arf;
            $datadaftar->a_no_awal_adrf = $request->no_urut_awal;
            $datadaftar->a_no_akhir_adrf = $request->no_urut_akhir;
            $datadaftar->user_id = $user_id;
            $datadaftar->save();
            $a_id_adrf = $datadaftar->a_id_adrf;

            for ($i=1; $i <= $a_jmlh_arf ; $i++) {
                $a_kd_brg_api = "$request->idBarang$i";
                $datadaftardetail = new AsetDaftarRuanganFakultasDetailModel();
                $datadaftardetail->a_id_adrf = $a_id_adrf;
    ...
    {      
        ...
        $datadaftar = AsetDaftarRuanganFakultasModel::where('a_id_adrf', $request->a_id_adrf)->first();
        $datadaftar->delete();
        $datadaftardetail = AsetDaftarRuanganFakultasDetailModel::where('a_id_adrf', $request->a_id_adrf);
