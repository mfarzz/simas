...
class PerubahanKoreksiRektoratKhususCt extends Controller
{
    public function index()
    {
        ...
        $datalokasi = AsetLokasiModel::where('id_fk', $datarektorat->id_ur)->first();
        ...
        return view('Sis_aset.Perubahan.Koreksi.Khusus.Rektorat.index',['daftar_kategori'=> $daftar_kategori]);
    }
    ...
    public function store(Request $request)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $jumlah_sppa = AsetUbahModel::where('a_kd_al', $datalokasi->a_kd_al)->where('a_kd_ajkt', 'B04')->where('a_thn_au', $thn)->count();
                    if($jumlah_sppa == 0)
                    {
                        $no_sppa = "1";
                    }
                    ...
                    $kdsppa = "B04$thn_kode$no_sppa";

                    $dataperubahan = new AsetUbahModel();
                    $dataperubahan->a_kd_al = $datalokasi->a_kd_al; 
                    $dataperubahan->a_kd_ajkt = "B04";
                    $dataperubahan->a_kd_brg = $request->idBarang;
                    $dataperubahan->a_kdsppa_au = $kdsppa;
                    $dataperubahan->a_nosppa_au = $no_sppa;
                    $dataperubahan->a_jmlh_au = ($request->nup_akhir - $request->nup_awal) + 1;
                    $dataperubahan->a_no_awal_au = $request->nup_awal;
                    $dataperubahan->a_no_akhir_au = $request->nup_akhir;
                    $dataperubahan->a_tgl_buku_au = $request->tgl_buku;
                    $dataperubahan->a_no_dasar_koreksi_au = $request->no_dasar_koreksi;
                    $dataperubahan->a_tgl_dasar_koreksi_au = $request->tgl_dasar_koreksi;
                    $dataperubahan->a_kuantitas_au = $request->kuantitas;
                    $dataperubahan->a_nilai_au = $request->nilai_aset;
                    $dataperubahan->a_thn_au = $thn;
                    $dataperubahan->a_periode_au = $bln;
                    $dataperubahan->user_id = $user_id;
                    $dataperubahan->save();
                    $a_id_au = $dataperubahan->a_id_au;
                    ...
                    for ($a=$request->nup_awal; $a <= $request->nup_akhir ; $a++) { 
                        ...
                        $dataperubahanitem = new AsetUbahItemModel();
                        $dataperubahanitem->a_id_au = $a_id_au;
                        $dataperubahanitem->a_kd_brg_api = $baris_aset_item->a_kd_brg_api;
                        $dataperubahanitem->a_no_api = $baris_aset_item->a_no_api;
                        $dataperubahanitem->a_kuantitas_aui = $request->kuantitas;
                        $dataperubahanitem->a_nilai_aui = $request->nilai_aset;
                        $dataperubahanitem->a_kuantitas_lama_aui = $baris_aset_item->a_kuantitas_api;
                        $dataperubahanitem->a_nilai_lama_aui = $baris_aset_item->a_nilai_api;
                        ...
                        $baris_aset_item_up->a_kuantitas_api = $request->kuantitas;
                        $baris_aset_item_up->a_nilai_api = $request->nilai_aset;
    ...
    {      
        ...
        $dataperubahanitem = AsetUbahItemModel::where('a_id_au', $request->a_id_au)->get();
        foreach($dataperubahanitem as $baris)
        {
            $baris_aset_item_up = AsetPerolehanItemModel::where('a_kd_brg_api',$baris->a_kd_brg_api)->first();
            $baris_aset_item_up->a_kuantitas_api = $baris->a_kuantitas_lama_aui;
            $baris_aset_item_up->a_nilai_api = $baris->a_nilai_lama_aui;
            $baris_aset_item_up->save();

            $dataperubahanitemdelete = AsetUbahItemModel::where('a_kd_brg_api', $baris->a_kd_brg_api);
            $dataperubahanitemdelete->delete();
        }

        $dataperubahan = AsetUbahModel::where('a_id_au', $request->a_id_au)->first();
