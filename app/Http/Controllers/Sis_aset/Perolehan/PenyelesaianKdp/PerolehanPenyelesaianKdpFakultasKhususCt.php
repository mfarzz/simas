...
namespace App\Http\Controllers\Sis_aset\Perolehan\PenyelesaianKdp;
...
class PerolehanPenyelesaianKdpFakultasKhususCt extends Controller
{
    public function index()
    {
        ...
        return view('Sis_aset.Perolehan.PenyelesaianKdp.Khusus.Fakultas.index',['daftar_kategori'=> $daftar_kategori]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $jumlah = AsetPerolehanModel::where('a_kd_al', $datalokasi->a_kd_al)->where('a_kd_ajkt', 'A19')->where('a_kd_brg', $request->idBarang)->where('a_tgl_perolehan_ap','>', $request->tgl_perolehan)->count();        
        if($jumlah>0)
        {
        ...
        {            
            $jumlah_barang = AsetPerolehanModel::where('a_kd_al', $datalokasi->a_kd_al)->where('a_kd_ajkt', 'A04')->where('a_kd_brg', $request->idBarang)->count();
            if($jumlah_barang == 0)
            {
                $no_awal_ap = "1";
            }
            ...
            {
                $cek_barang = AsetPerolehanModel::where('a_kd_al', $datalokasi->a_kd_al)->where('a_kd_ajkt', 'A04')->where('a_kd_brg', $request->idBarang)->count();
            ...
            $datahibah->a_kondisi_ap = $request->kondisi;
