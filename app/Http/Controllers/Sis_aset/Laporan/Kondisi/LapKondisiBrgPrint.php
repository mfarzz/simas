...
use App\Models\User;
use App\Models\VAsetLapTransKategoriBrgKondisiModel;
use App\Models\VAsetLapTransKategoriBrgModel;
use App\Models\VAsetLapTransKategoriKondisiModel;
use App\Models\VAsetLapTransKategoriModel;
use Illuminate\Support\Facades\Crypt;
use PDF;

class LapKondisiBrgPrint extends Controller
{
    public function index($tgl_akhir, $kondisi, $id_lokasi)
    {        
        $tgl_akhir = Crypt::decryptString($tgl_akhir);
        $kondisi = Crypt::decryptString($kondisi);
        $a_kd_al = Crypt::decryptString($id_lokasi);

        $user_id = auth()->user()->id;
        $datalokasi = AsetLokasiModel::where('a_kd_al', $a_kd_al)->first();

        $jumlah = AsetTempLapModel::where('user_id', $user_id)->where('a_jns_atl','=','2')->count();
        if($jumlah != 0)
        {
            $datadeletetbm = AsetTempLapModel::where('user_id', $user_id)->where('a_jns_atl','=','2');
        ...
        ->where('a_tgl_perolehan_ap', '<=', $tgl_akhir)
        ...
        {            
            ...
            $dataasettemp->a_kd_brg = $barisperolehan->a_kd_brg_api;
            $dataasettemp->a_jmlh_atl = $barisperolehan->a_kuantitas_api;
            $dataasettemp->a_hrg_atl = $barisperolehan->a_nilai_api;
            $dataasettemp->a_kd_al = $datalokasi->a_kd_al;
            $dataasettemp->a_jns_atl = 2;
            $dataasettemp->a_knds_atl = $barisperolehan->a_kondisi_api;
        ...
        PDF::SetTitle('Laporan Kondisi Barang');        
        PDF::AddPage('L');
        $tgl_akhir = \Carbon\Carbon::parse($tgl_akhir)->format('d M Y');

        PDF::SetFont('times', 'b', 10);
        PDF::Cell(0, 0, 'LAPORAN KONDISI BARANG', 0, 1, 'C', 0, '', 0);
        PDF::SetFont('times', 'b', 10);        
        PDF::Cell(0, 0, "S/D $tgl_akhir", 0, 1, 'C', 0, '', 0);
        ...
        PDF::ln(5);
        PDF::Cell(15, 0, "No.", 1, 0, 'C', 0, '', true);
        PDF::Cell(130, 0, "Sub-Sub Kelompok", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "NUP", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "Satuan", 1, 0, 'C', 0, '', true);
        PDF::Cell(30, 0, "Harga Perolehan", 1, 0, 'C', 0, '', true);
        PDF::Cell(60, 0, "Kondisi", 1, 1, 'C', 0, '', true);
        PDF::ln(0);        
        PDF::Cell(15, 0, "", 1, 0, 'C', 0, '', true);
        PDF::Cell(30, 0, "Kode Barang", 1, 0, 'C', 0, '', true);
        PDF::Cell(100, 0, "Nama Barang", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
        PDF::Cell(30, 0, "", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "Baik", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "Rusak Ringan", 1, 0, 'C', 0, '', true);
        PDF::Cell(20, 0, "Rusak Berat", 1, 1, 'C', 0, '', true);

        $total_nilai = 0;
        $no = 1;
        $datalap = VAsetLapTransKategoriKondisiModel::
        where('v_aset_lap_trans_kategori_kondisi.a_kd_al','=', "$datalokasi->a_kd_al")
        ->where('v_aset_lap_trans_kategori_kondisi.user_id','=',$user_id)
        ->where('v_aset_lap_trans_kategori_kondisi.a_jns_atl','=',2)
        ->get();
        foreach($datalap as $barislap)
        {
            ...
            PDF::Cell(15, 0, "$no", 1, 0, 'C', 0, '', true);
            PDF::Cell(30, 0, "$barislap->a_kd_kt", 1, 0, 'L', 0, '', true);
            PDF::Cell(100, 0, "$barislap->a_nm_kt", 1, 0, 'L', 0, '', true);
            PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
            PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
            PDF::Cell(30, 0, "", 1, 0, 'C', 0, '', true);
            PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
            PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
            PDF::Cell(20, 0, "", 1, 1, 'C', 0, '', true); 
            $datalapbrg = VAsetLapTransKategoriBrgKondisiModel::
            join('aset_perolehan_item','v_aset_lap_trans_kategori_brg_kondisi.a_kd_brg_api','=','aset_perolehan_item.a_kd_brg_api')
            ->where('v_aset_lap_trans_kategori_brg_kondisi.a_kd_kt','=', "$barislap->a_kd_kt")            
            ->where('v_aset_lap_trans_kategori_brg_kondisi.a_kd_al','=', "$datalokasi->a_kd_al")
            ->where('v_aset_lap_trans_kategori_brg_kondisi.user_id','=',$user_id)
            ->where('v_aset_lap_trans_kategori_brg_kondisi.a_jns_atl','=',2)
            ->orderBy('v_aset_lap_trans_kategori_brg_kondisi.a_kd_brg_api')
            ->get();
            foreach($datalapbrg as $barisbrg)
            {
                $a_nilai_api = rupiah($barisbrg->a_nilai_api);
                PDF::SetFont('times', '', 10);
                PDF::SetTextColor(0, 0, 0);
                PDF::Cell(15, 0, "$no", 1, 0, 'C', 0, '', true);
                PDF::Cell(30, 0, "$barisbrg->a_kd_brg_api", 1, 0, 'L', 0, '', true);
                PDF::Cell(100, 0, "$barisbrg->a_nm_brg", 1, 0, 'L', 0, '', true);
                PDF::Cell(20, 0, "$barisbrg->a_no_api", 1, 0, 'C', 0, '', true);
                PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
                PDF::Cell(30, 0, "$a_nilai_api", 1, 0, 'R', 0, '', true);
                PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
                PDF::Cell(20, 0, "", 1, 0, 'C', 0, '', true);
                PDF::Cell(20, 0, "", 1, 1, 'C', 0, '', true); 
                $no++;
