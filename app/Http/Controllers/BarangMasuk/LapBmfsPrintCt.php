...
class LapBmfsPrintCt extends Controller
{
    Public Function index($filter, $filter2)
    {
        ...
        PDF::SetFooterCallback($footer);
            ...
            join('barang_masuk_fakultas_pesanan','barang_masuk_fakultas.id_bmfp','=','barang_masuk_fakultas_pesanan.id_bmfp')
            ->join('barang','barang_masuk_fakultas.kd_brg','=','barang.kd_brg')
            ->join('kategori','barang.kd_kt','=','kategori.kd_kt')
            ->where('barang_masuk_fakultas.id_fk','=',$datafakultas->id_fk)
            ->where('tglbuku_bmf', '<=', $tgl_akhir )
            ->where('tglbuku_bmf', '>=', $tgl_awal )
            ->get();
            foreach($datalap as $barislap)
            {
                $hrg_bmf_rp = rupiah($barislap->hrg_bmf);
                $total = $barislap->hrg_bmf * $barislap->jmlh_awal_bmf;
                ...
                PDF::Cell(25, 0, "$barislap->jmlh_awal_bmf", 1, 0, 'C', 0, '', true);
