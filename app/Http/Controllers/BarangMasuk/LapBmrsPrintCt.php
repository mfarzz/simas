...
class LapBmrsPrintCt extends Controller
{
    Public Function index($filter, $filter2)
    {
        ...
        PDF::SetFooterCallback($footer);
            $total_nilai = 0;
            $no=1;
            $datalap = BarangMasukRektoratModel::
            join('barang_masuk_rektorat_pesanan','barang_masuk_rektorat.id_bmrp','=','barang_masuk_rektorat.id_bmrp')
            ->join('barang','barang_masuk_rektorat.kd_brg','=','barang.kd_brg')
            ->join('kategori','barang.kd_kt','=','kategori.kd_kt')
            ->where('barang_masuk_rektorat.id_ur','=',$datarektorat->id_ur)
            ->where('tglbuku_bmr', '<=', $tgl_akhir )
            ->where('tglbuku_bmr', '>=', $tgl_awal )
            ->get();
            foreach($datalap as $barislap)
            {
                $hrg_bmr_rp = rupiah($barislap->hrg_bmr);
                $total = $barislap->hrg_bmr * $barislap->jmlh_awal_bmr;
                ...
                PDF::Cell(25, 0, "$barislap->jmlh_awal_bmr", 1, 0, 'C', 0, '', true);
