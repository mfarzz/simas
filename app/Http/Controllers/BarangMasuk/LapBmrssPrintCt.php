...
class LapBmrssPrintCt extends Controller
{
    Public Function index($filter, $filter2)
    {
        ...
        $header = function() use ($tgl_akhir, $tahunanggaran, $datalokasi, $lokasi, $tgl_awal) {
            ...
            PDF::Cell(0, 0, 'LAPORAN BARANG MASUK', 0, 1, 'C', 0, '', 0);
            ...
            PDF::Cell(20, 0, "No SP2D", 1, 0, 'C', 0, '', true);
        ...
        PDF::SetFooterCallback($footer);
            $total_nilai = 0;
            $no=1;
            $datalap = BarangMasukRumahSakitModel::
            join('barang_masuk_rumah_sakit_pesanan','barang_masuk_rumah_sakit.id_bmrsp','=','barang_masuk_rumah_sakit_pesanan.id_bmrsp')
            ->join('barang','barang_masuk_rumah_sakit.kd_brg','=','barang.kd_brg')
            ->join('kategori','barang.kd_kt','=','kategori.kd_kt')
            ->where('barang_masuk_rumah_sakit.id_urs','=',$datarumahsakit->id_urs)
            ->where('tglbuku_bmrs', '<=', $tgl_akhir )
            ->where('tglbuku_bmrs', '>=', $tgl_awal )
            ->get();
            foreach($datalap as $barislap)
            {
                $hrg_bmrs_rp = rupiah($barislap->hrg_bmrs);
                $total = $barislap->hrg_bmrs * $barislap->jmlh_awal_bmrs;
                $total_rp = rupiah($total);
                PDF::SetFont('times', '', 10);
                PDF::Cell(10, 0, "$no", 1, 0, 'C', 0, '', true);
                PDF::Cell(20, 0, "$barislap->no_sp2d", 1, 0, 'C', 0, '', true);
                PDF::Cell(80, 0, "$barislap->nm_kt", 1, 0, 'L', 0, '', true);
                PDF::Cell(80, 0, "$barislap->nm_brg", 1, 0, 'L', 0, '', true);
                PDF::Cell(25, 0, "$barislap->jmlh_awal_bmrs", 1, 0, 'C', 0, '', true);
                PDF::Cell(30, 0, "$hrg_bmrs_rp", 1, 0, 'R', 0, '', true);
