            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Nomor OPSIK : {{ $cek_opurs->no_opurs }}</p>
                        <p>Tanggal OPSIK : {{ $tgl = \Carbon\Carbon::parse($cek_opurs->tgl_opurs)->formatLocalized('%d %B %Y'); }}</p>
                        <p>Semester : {{ $cek_opurs->sem_opurs }}</p>
                        <p>Tahun Anggaran : {{ $cek_opurs->thn_opurs }}</p>
                        <p>Status : @if($cek_opurs->status_opurs==0) Draf @elseif($cek_opurs->status_opurs == 1)Kirim @endif</p>
                      </div>
                    </div>
                </div>
                ...
                <a href="/opsik-oprs"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar opname fisik"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Kembali Ke Daftar Opname Fisik </button></a>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_opursdet" id="id_opursdet">
...
$('#kategori').change(function(){ 
    if(isTambahData){   
        ...
        if(Kat){
            $.ajax({
            type:"GET",
            url:"/opsik-oprs-detail-item?item="+Kat,
            dataType: 'JSON',
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        ...
        ajax: "{{ url('opsik-oprs-detail', ['encripted_id' => $encripted_id]) }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'kd_brg', name: 'kd_brg' },
            { 
                data: 'stok_sistem_opursdet', 
                "render": function(data) {
                    return '<div class="centered-cell">' + data + '</div>';
                }
            },    
            { 
                data: 'stok_opsik_opursdet',
                "render": function(data) {
            ...
            {data: 'action',
                ...
                render: function(data, type, row) {
                    ...
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_opursdet + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_opursdet + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    }
                    return buttons;
                }
            },
...
function editFunc(id_opursdet){  
    isTambahData = false;  
    $.ajax({
        type:"POST",
        url: "{{ url('opsik-oprs-detail-edit') }}",
        data: { id_opursdet: id_opursdet },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_opursdet').val(res.id_opursdet);
            $('#kategori').val(res.kd_kl).trigger('change');
            $('#id_opursdet').val(res.id_opursdet);
            $('#stok_fisik').val(res.stok_opsik_opursdet);

            $.ajax({
                type: "GET",
                url: "/opsik-oprs-detail-item?item=" + res.kd_kt,
                dataType: 'JSON',
...
function deleteFunc(id_opursdet){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_opursdet = id_opursdet;
        $.ajax({
            type:"POST",
            url: "{{ url('opsik-oprs-detail-delete') }}",
            data: { id_opursdet : id_opursdet },
            dataType: 'json',
            success: function(res){    
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('opsik-oprs-detail-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
