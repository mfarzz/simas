        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_opur" id="id_opur">
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">No OPSIK</label>
                        <div class="col-sm-8">
                          <input type="text" id="no_opur" name="no_opur" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Nomor OPSIK" required>
                        </div>
                    </div>
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Tanggal OPSIK</label>
                        <div class="col-sm-8">
                          <input type="date" id="tgl_opur" name="tgl_opur" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Tanggal OPSIK" required>
                        </div>
                    </div>
                    <div class="row mb-4">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataFormUpload" name="DataFormUpload" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_opur_cek" id="id_opur_cek">
                    <div class="row mb-4">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        ...
        columns: [
            ...
            { data: 'sem_opur', name: 'sem_opur' },
            { data: 'thn_opur', name: 'thn_opur' },
            {
                data: 'status_opur',
                name: 'status_opur',
                render: function (data, type, row) {
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    if (row.status_opur == 0) {
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_opur + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_opur + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ' +                    
                    '<a href="javascript:void(0);" onClick="validasiFunc(' + row.id_opur + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Validasi Data" title="Validasi Data" class="delete btn btn-primary btn-rounded btn-sm waves-effect"><i class="fa fa-check fa-sm"></i></a> ';
                    }
                    else
                    {
                        buttons +=                         
                        '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Upload Data" onClick="uploadFunc(' + row.id_opur + ')" data-original-title="Upload" class="edit btn btn-primary btn-rounded btn-sm waves-effect"><i class="fa fa-upload fa-sm"></i></a> ' +
                        '<a href="opsik-opr-lampiran/' + row.id_opur_en + '" target="_blank" data-bs-toggle="tooltip" data-bs-placement="left" title="Cetak Lampiran" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-print fa-sm"></i></a> ' +
                        '<a href="opsik-opr-persediaan/' + row.id_opur_en + '" target="_blank" data-bs-toggle="tooltip" data-bs-placement="left" title="Cetak Laporan Persediaan" class="btn btn-rounded btn-sm btn-warning"><i class="fa fa-print fa-sm"></i></a> ';
                    }
                    buttons += 
                    '<a href="opsik-opr-detail/' + row.id_opur_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Daftar Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> ' ;
...
function editFunc(id_opur){    
    $.ajax({
        type:"POST",
        url: "{{ url('opsik-opr-edit') }}",
        data: { id_opur: id_opur },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_opur').val(res.id_opur);            
            $('#no_opur').val(res.no_opur);
            $('#tgl_opur').val(res.tgl_opur);
            $('#sem_opur').val(res.sem_opur);
            $('#thn_opur').val(res.thn_opur);
        }
    });
}  
 
function deleteFunc(id_opur){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_opur = id_opur;
        $.ajax({
            type:"POST",
            url: "{{ url('opsik-opr-delete') }}",
            data: { id_opur: id_opur },
            dataType: 'json',
            success: function(res){    
...
function validasiFunc(id_opur){
    var pesan = "Apakah anda yakin akan validasi data ini ?";
    if (confirm(pesan) == true) {
        var id_opur = id_opur;
        $.ajax({
            type:"POST",
            url: "{{ url('opsik-opr-validasi') }}",
            data: { id_opur: id_opur },
            dataType: 'json',
            success: (data) => {  
                if (data.status === 1) {
...
function uploadFunc(id_opur){
    $.ajax({
        type:"POST",
        url: "{{ url('opsik-opr-store-upload-cek') }}",
        data: { id_opur: id_opur },
        dataType: 'json',
        success: function(res){
            ...
            $('#id_opur_cek').val(res.id_opur);
            if (res.file_opur) {           
                $('#label-dokumen').text('Change PDF'); // Ganti teks label
                var pdfPath = window.location.origin + "/storage/berita_acara/" + res.file_opur;
                loadPDF(pdfPath);
            } else {
...
$('#DataFormUpload').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('opsik-opr-store-upload')}}",
        data: formDataUpload,
        cache:false,
        contentType: false,
        processData: false,
