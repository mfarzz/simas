            ...
            <div class="d-inline-block align-items-center">
                <nav>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pengguna Rektorat</li>
                    </ol>
                </nav>
            </div>
        </div>        
        ...
        <div class="col-12">                
            <div class="box">
                <div class="box-body ribbon-box">
                    <div class="ribbon ribbon-success">Daftar Pengguna Rektorat
                    </div>                    
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Bagian Rektorat</label>
                        <div class="col-sm-8">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Jabatan Bagian Rektorat</label>
                        <div class="col-sm-8">
...
$('#idUnit').change(function(){   
    ...
    if(unit){
        $.ajax({
           type:"GET",
           url:"/master-pengguna-rektorat-unitjabatan?unitjabatan="+unit,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idUnitjabatan").empty();
                $("#idUnitjabatan").append("<option value=''>---Silahkan Pilih Jabatan di Bagian Rektorat---</option>");
                $.each(res,function(nm_urj,id_urj){
                    $("#idUnitjabatan").append('<option value="'+id_urj+'">'+nm_urj+'</option>');
                });
            }else{
               $("#idUnitjabatan").empty();
            }
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('master-pengguna-rektorat') }}",
        columns: [
...
function editFunc(id){    
    $.ajax({
        type:"POST",
        url: "{{ url('master-pengguna-rektorat-edit') }}",
        data: { id: id },
        dataType: 'json',
        success: function(res){
            ...
            $('#no_wa').val(res.nowa);
            $('#email').val(res.email);
            $('#idUnit').val(res.id_ur);
            $('#pengguna').val(res.pengguna);
            $.ajax({
                type: "GET",
                url: "/master-pengguna-rektorat-unitjabatan?unitjabatan=" + res.id_ur,
                dataType: 'JSON',
                success: function(item) {
                    $("#idUnitjabatan").empty();
                    $("#idUnitjabatan").append("<option value=''>---Silahkan Pilih Jabatan di Bagian Rektorat---</option>");
                    $.each(item, function(nm_urj, id_urj) {
                        $("#idUnitjabatan").append('<option value="' + id_urj + '">' + nm_urj + '</option>');
                    });
                    
                    $('#idUnitjabatan').val(res.id_urj).trigger('change');
                }
            });
        }
    });
...
function deleteFunc(id){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('master-pengguna-rektorat-delete') }}",
            data: { id: id },
            dataType: 'json',
            success: function(res){    
...
function resetFunc(id){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('master-pengguna-rektorat-reset') }}",
            data: { id: id },
            dataType: 'json',
            success: function(res){    
                var oTable = $('#data-datatable').dataTable();
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-pengguna-rektorat-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
