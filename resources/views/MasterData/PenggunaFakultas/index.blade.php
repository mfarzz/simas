            ...
            <div class="d-inline-block align-items-center">
                <nav>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pengguna Fakultas</li>
                    </ol>
                </nav>
            </div>
        </div>        
        ...
        <div class="col-12">                
            <div class="box">
                <div class="box-body ribbon-box">
                    <div class="ribbon ribbon-success">Daftar Pengguna Fakultas
                    </div>                    
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-8">
                            <select id="idFakultas" name="idFakultas" class="form-select form-control" aria-label="Default select example" >
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Jabatan Fakultas</label>
                        <div class="col-sm-8">
                            <select id="idFakultasjabatan" name="idFakultasjabatan" class="form-select form-control" aria-label="Default select example" required>
...
$('#idFakultas').change(function(){   
    ...
    if(fakultas){
        $.ajax({
           type:"GET",
           url:"/master-pengguna-fakultas-jabatan?fakultasjabatan="+fakultas,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idFakultasjabatan").empty();
                $("#idFakultasjabatan").append("<option value=''>---Silahkan Pilih Jabatan di Fakultas---</option>");
                $.each(res,function(nm_fkj,id_fkj){
                    $("#idFakultasjabatan").append('<option value="'+id_fkj+'">'+nm_fkj+'</option>');
                });
            }else{
               $("#idFakultasjabatan").empty();
            }
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('master-pengguna-fakultas') }}",
        columns: [
...
function editFunc(id){    
    $.ajax({
        type:"POST",
        url: "{{ url('master-pengguna-fakultas-edit') }}",
        data: { id: id },
        dataType: 'json',
        success: function(res){
            ...
            $('#no_wa').val(res.nowa);
            $('#email').val(res.email);
            $('#idFakultas').val(res.id_fk);
            $('#pengguna').val(res.pengguna);
            $.ajax({
                type: "GET",
                url: "/master-pengguna-fakultas-jabatan?fakultasjabatan=" + res.id_fk,
                dataType: 'JSON',
                success: function(item) {
                    $("#idFakultasjabatan").empty();
                    $("#idFakultasjabatan").append("<option value=''>---Silahkan Pilih Jabatan di Fakultas---</option>");
                    $.each(item, function(nm_fkj, id_fkj) {
                        $("#idFakultasjabatan").append('<option value="' + id_fkj + '">' + nm_fkj + '</option>');
                    });
                    
                    $('#idFakultasjabatan').val(res.id_fkj).trigger('change');
                }
            });
        }
    });
...
function deleteFunc(id){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('master-pengguna-fakultas-delete') }}",
            data: { id: id },
            dataType: 'json',
            success: function(res){    
...
function resetFunc(id){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('master-pengguna-fakultas-reset') }}",
            data: { id: id },
            dataType: 'json',
            success: function(res){    
                var oTable = $('#data-datatable').dataTable();
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-pengguna-fakultas-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
