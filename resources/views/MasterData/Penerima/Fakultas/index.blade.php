        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bkpf" id="id_bkpf">
                    <div class="row mb-4">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar-penerima-opf') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bkpf + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bkpf + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    return buttons;
                }
            },     
        ],
...
function editFunc(id_bkpf){    
    $.ajax({
        type:"POST",
        url: "{{ url('barang-keluar-penerima-opf-edit') }}",
        data: { id_bkpf: id_bkpf },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bkpf').val(res.id_bkpf);
            $('#nama').val(res.nm_bkpf);
            $('#status').val(res.status_bkpf);
        }
    });
}  
 
function deleteFunc(id_bkpf){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_bkpf = id_bkpf;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-keluar-penerima-opf-delete') }}",
            data: { id_bkpf: id_bkpf },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-penerima-opf-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
