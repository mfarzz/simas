        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bkprs" id="id_bkprs">
                    <div class="row mb-4">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar-penerima-oprs') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bkprs + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bkprs + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    return buttons;
                }
            },     
        ],
...
function editFunc(id_bkprs){    
    $.ajax({
        type:"POST",
        url: "{{ url('barang-keluar-penerima-oprs-edit') }}",
        data: { id_bkprs: id_bkprs },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bkprs').val(res.id_bkprs);
            $('#nama').val(res.nm_bkprs);
            $('#status').val(res.status_bkprs);
        }
    });
}  
 
function deleteFunc(id_bkprs){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_bkprs = id_bkprs;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-keluar-penerima-oprs-delete') }}",
            data: { id_bkprs: id_bkprs },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-penerima-oprs-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
