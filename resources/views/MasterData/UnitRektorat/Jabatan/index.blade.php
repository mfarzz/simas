            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Unit Rektorat : {{ $data_unitrektorat->nm_ur }} </p>
                      </div>
                    </div>
                </div>
                <div class="box-body">                  
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,        
        ajax: "{{ url('master-unitrektorat-jabatan', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_urj + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" onClick="deleteFunc(' + row.id_urj + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> '
                    ;
                    return buttons;
                }
            },
...
function editFunc(id_urj){
    $.ajax({
        type:"POST", 
        url: "{{ url('master-unitrektorat-jabatan-edit') }}",
        data: { id_urj : id_urj },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_urj').val(res.id_urj);
            $('#role').val(res.role_id);
        }
    });
}  
 
function deleteFunc(id_urj){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_urj = id_urj;
        $.ajax({
            type:"POST",
            url: "{{ url('master-unitrektorat-jabatan-delete') }}",
            data: { id_urj : id_urj },
            dataType: 'json',
            success: function(res){    
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-unitrektorat-jabatan-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
