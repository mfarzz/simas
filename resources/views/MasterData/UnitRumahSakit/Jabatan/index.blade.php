            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Unit Rumah Sakit : {{ $data_unitrumahsakit->nm_urs }} </p>
                      </div>
                    </div>
                </div>
                <div class="box-body">                  
                ...
                <a href="/master-unitrumahsakit"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar unit rektorat"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Kembali Ke Daftar Unit Rumah Sakit </button></a>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_ursj" id="id_ursj">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,        
        ajax: "{{ url('master-unitrumahsakit-jabatan', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_ursj + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" onClick="deleteFunc(' + row.id_ursj + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> '
                    ;
                    return buttons;
                }
            },
...
function editFunc(id_ursj){
    $.ajax({
        type:"POST", 
        url: "{{ url('master-unitrumahsakit-jabatan-edit') }}",
        data: { id_ursj : id_ursj },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_ursj').val(res.id_ursj);
            $('#role').val(res.role_id);
        }
    });
}  
 
function deleteFunc(id_ursj){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_ursj = id_ursj;
        $.ajax({
            type:"POST",
            url: "{{ url('master-unitrumahsakit-jabatan-delete') }}",
            data: { id_ursj : id_ursj },
            dataType: 'json',
            success: function(res){    
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-unitrumahsakit-jabatan-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
