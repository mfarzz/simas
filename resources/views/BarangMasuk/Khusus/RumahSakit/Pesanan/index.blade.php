        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bmrsp" id="id_bmrsp">
                    <div class="row mb-4">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">No Pesanan</label>
                        <div class="col-sm-8">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Tanggal Pesanan</label>
                        <div class="col-sm-8">
...
<script type="text/javascript">
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-masuk-pesanan-oprs-khusus-lap')}}",
        ...
        success: (data) => {  
            ...
            var link = "/barang-masuk-pesanan-oprs-khusus-lap-print/" + encodeURIComponent(tgl_awal) + "/" + encodeURIComponent(tgl_akhir);
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-masuk-pesanan-oprs-khusus') }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'no_sp2d', name: 'no_sp2d' },
            { data: 'nm_mak', name: 'nm_mak' },
            { data: 'no_bmrsp', name: 'no_bmrsp' },
            {
                data: 'tgl_bmrsp',
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bmrsp + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bmrsp + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ' +                    
                    '<a href="javascript:void(0);" onClick="validasiFunc(' + row.id_bmrsp + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Validasi Data" title="Validasi Data" class="delete btn btn-primary btn-rounded btn-sm waves-effect"><i class="fa fa-check fa-sm"></i></a> ';
                    }
                    buttons +=  
                    '<a href="barang-masuk-oprs-khusus/' + row.id_bmrsp_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Daftar Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> ';
                    return buttons;
                }
            },     
        ],
...
function editFunc(id_bmrsp){    
    $.ajax({
        type:"POST",
        url: "{{ url('barang-masuk-pesanan-oprs-khusus-edit') }}",
        data: { id_bmrsp: id_bmrsp },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bmrsp').val(res.id_bmrsp);   
            $('#no_sp2d').val(res.no_sp2d);
            $('#kd_mak').val(res.kd_mak);         
            $('#no_pesanan').val(res.no_bmrsp);
            $('#tgl_pesanan').val(res.tgl_bmrsp);
            $('#nilai_pesanan').val(res.nilai_bmrsp);
            $('#nilai_pesanan2').val(formatRupiah2(res.nilai_bmrsp));
        }
    });
}  
 
function deleteFunc(id_bmrsp){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_bmrsp = id_bmrsp;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-masuk-pesanan-oprs-khusus-delete') }}",
            data: { id_bmrsp: id_bmrsp },
            dataType: 'json',
            success: function(res){    
...
function validasiFunc(id_bmrsp){
    var pesan = "Apakah anda yakin akan validasi data ini ?";
    if (confirm(pesan) == true) {
        var id_bmrsp = id_bmrsp;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-masuk-pesanan-oprs-khusus-validasi') }}",
            data: { id_bmrsp: id_bmrsp },
            dataType: 'json',
            success: (data) => {  
                if (data.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-masuk-pesanan-oprs-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
