...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-masuk-sp2d-opr-khusus') }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'no_bmrs', name: 'no_bmrs' },
            {
                data: 'tgl_bmrs',
            ...
            { data: 'nilai_bmrs', name: 'nilai_bmrs' },
            {
                data: 'status_bmrs',
                name: 'status_bmrs',
                render: function (data, type, row) {
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    '<a href="javascript:void(0);" onClick="validasiFunc(' + row.id_bmrs + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Validasi Data" title="Validasi Data" class="delete btn btn-primary btn-rounded btn-sm waves-effect"><i class="fa fa-check fa-sm"></i></a> ';
                    }
                    buttons += 
                    '<a href="barang-masuk-opr-khusus/' + row.id_bmrs_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Daftar Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> ';
...
function editFunc(id_bmrs){    
    $.ajax({
        type:"POST",
        url: "{{ url('barang-masuk-sp2d-opr-khusus-edit') }}",
        ...
        success: function(res){
            ...
            $('#no_sp2d').val(res.no_bmrs);
            $('#tgl_sp2d').val(res.tgl_bmrs);
            $('#nilai_sp2d').val(res.nilai_bmrs);
        }
    });
}  
...
function deleteFunc(id_bmrs){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('barang-masuk-sp2d-opr-khusus-delete') }}",
            data: { id_bmrs: id_bmrs },
            dataType: 'json',
            success: function(res){    
...
function validasiFunc(id_bmrs){
    var pesan = "Apakah anda yakin akan validasi data ini ?";
    if (confirm(pesan) == true) {
        var id_bmrs = id_bmrs;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-masuk-sp2d-opr-khusus-validasi') }}",
            data: { id_bmrs: id_bmrs },
            dataType: 'json',
            success: (data) => {  
                if (data.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-masuk-sp2d-opr-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
