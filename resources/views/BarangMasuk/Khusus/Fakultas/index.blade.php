            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Nomor Kontrak/Kuitansi : {{ $cek_bmfp->no_bmfp }}</p>
                        <p>Tanggal Kontrak/Kuitansi : {{ $tgl = \Carbon\Carbon::parse($cek_bmfp->tgl_bmfp)->locale('id')->isoFormat('D MMMM Y'); }}</p>
                        <p>Nilai Pesanan : @currency($cek_bmfp->nilai_bmfp)</p>
                        <p>Total Nilai Item Barang : @currency($total_hrg)</p>
                        <p>Status : @if($cek_bmfp->status_bmfp==0) Draf @elseif($cek_bmfp->status_bmfp == 1)Kirim @endif</p>
                      </div>
                    </div>
                </div>
                ...
                <a href="/barang-masuk-pesanan-opf-khusus"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar barang keluar nota"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Kembali Ke Daftar Barang Masuk Pesanan </button></a>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bmf" id="id_bmf">
...
$('#kategori').change(function(){     
    if(isTambahData){   
        ...
        if(kat){
            $.ajax({
            type:"GET",
            url:"/barang-masuk-opf-khusus-item?item="+kat,
            dataType: 'JSON',
...
$(document).ready( function () {
    ...
    // Fungsi format untuk menambahkan titik sebagai pemisah ribuan dan mempertahankan koma sebagai pemisah desimal
    ...
    $('#data-datatable').DataTable({
        ...
        ajax: "{{ url('barang-masuk-opf-khusus', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            {data: 'action',
                ...
                render: function(data, type, row) {
                    ...
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bmf + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bmf + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    }
                    return buttons;
                }
            },
...
function editFunc(id_bmf){  
    isTambahData = false;  
    $.ajax({
        type:"POST",
        url: "{{ url('barang-masuk-opf-khusus-edit') }}",
        data: { id_bmf: id_bmf },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bmf').val(res.id_bmf);
            $('#kategori').val(res.kd_kt).trigger('change');
            $('#idBarang').val(res.kd_brg).trigger('change');
            $('#id_bmf').val(res.id_bmf);
            $('#jumlah').val(res.jmlh_awal_bmf);
            $('#harga2').val(res.hrg_bmf);
            $('#harga').val(res.hrg_bmf);
            $('#tgl_perolehan').val(res.tglperolehan_bmf);

            $.ajax({
                type: "GET",
                url: "/barang-masuk-opf-khusus-item?item=" + res.kd_kt,
                dataType: 'JSON',
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-masuk-opf-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
