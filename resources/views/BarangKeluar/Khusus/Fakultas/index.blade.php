            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Nomor Nota : {{ $cek_bkfn->no_bkfn }}</p>
                        <p>Tanggal Nota : {{ $tgl = \Carbon\Carbon::parse($cek_bkfn->tgl_bkfn)->formatLocalized('%d %B %Y'); }}</p>
                        <p>Status : @if($cek_bkfn->status_bkfn==0) Draf @elseif($cek_bkfn->status_bkfn == 1)Kirim @endif</p>
                      </div>
                    </div>
                </div>
                <div class="box-body">    
                ...
                <a href="/barang-keluar-nota-opf-khusus"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar barang keluar nota"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Daftar Nota Barang Keluar (Pemakaian)  </button></a>
            </div>
        </div>
    </div>
</section>
...
$('#kategori').change(function(){ 
    if(isTambahData){  
        ...
        if(Kat){
            $.ajax({
            type:"GET",
            url:"/barang-keluar-opf-khusus-item?item="+Kat,
            dataType: 'JSON',
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar-opf-khusus', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (row.status_bkfn == 0) {
                        buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bkf + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                            '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bkf + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    }
                    return buttons;
                }
            },
...
function editFunc(id_bkf){  
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            $.ajax({
                type: "GET",
                url: "/barang-keluar-opf-khusus-item?item=" + res.kd_kt,
                dataType: 'JSON',
