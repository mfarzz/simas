        ...
        <div class="col-12">                
            <div class="box">
                <div class="box-body ribbon-box">
                    <div class="ribbon ribbon-success">Daftar Nota Barang Keluar (Pemakaian)
                    </div>                    
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bkfn" id="id_bkfn">
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-8">
                            <select id="penerima" name="penerima" class="form-select form-control" required>
                                ...
                                @foreach ($daftar_penerima as $baris )
                                    <option value={{ $baris->id_bkpf }}> {{ $baris->nm_bkpf }} </option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-nota-opf-khusus-lap')}}",
        ...
        success: (data) => {  
            ...
            var link = "/barang-keluar-nota-opf-khusus-lap-print/" + encodeURIComponent(tgl_awal) + "/" + encodeURIComponent(tgl_akhir);
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar-nota-opf-khusus') }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'nm_bkpf', name: 'nm_bkpf' },
            { data: 'no_bkfn', name: 'no_bkfn' },
            {
                data: 'tgl_bkfn',
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    if (row.status_bkfn == 0) {
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bkfn + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bkfn + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ' +                    
                    '<a href="javascript:void(0);" onClick="validasiFunc(' + row.id_bkfn + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Validasi Data" title="Validasi Data" class="delete btn btn-primary btn-rounded btn-sm waves-effect"><i class="fa fa-check fa-sm"></i></a> ';
                    }
                    buttons += 
                                
                    '<a href="barang-keluar-opf-khusus/' + row.id_bkfn_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Daftar Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> ';
...
function editFunc(id_bkfn){    
    $.ajax({
        type:"POST",
        url: "{{ url('barang-keluar-nota-opf-khusus-edit') }}",
        data: { id_bkfn: id_bkfn },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bkfn').val(res.id_bkfn);
            $('#penerima').val(res.id_bkpf);
            $('#no_nota').val(res.no_bkfn);
            $('#tgl_nota').val(res.tgl_bkfn);
        }
    });
}  
 
function deleteFunc(id_bkfn){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var id_bkfn = id_bkfn;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-keluar-nota-opf-khusus-delete') }}",
            data: { id_bkfn: id_bkfn },
            dataType: 'json',
            success: function(res){    
...
function validasiFunc(id_bkfn){
    var pesan = "Apakah anda yakin akan validasi data ini ?";
    if (confirm(pesan) == true) {
        var id_bkfn = id_bkfn;
        $.ajax({
            type:"POST",
            url: "{{ url('barang-keluar-nota-opf-khusus-validasi') }}",
            data: { id_bkfn: id_bkfn },
            dataType: 'json',
            success: (data) => {  
                if (data.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-nota-opf-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
