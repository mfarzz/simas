    ...
    <div class="table-rep-plugin">
        <div class="table-responsive mb-0" data-pattern="priority-columns">
            <table id="tech-companies-1" class="table table-striped">
                ...
                <tbody>
                @foreach ($databarangkeluar as $valuebk)
                    <tr>
                        <td align='center' style='border:1px solid #90AFC5; color:black'>{{ ++$no }}</td>
                        <td style=' border:1px solid #90AFC5 ; color:black'>{{ \Carbon\Carbon::parse($valuebk->tglbeli_bm)->format('d M Y')}}</td> 
                        <td style=' border:1px solid #90AFC5 ; color:black'>@currency($valuebk->hrg_bm)</td>
                        <td style=' border:1px solid #90AFC5 ; color:black'>{{ $valuebk->jmlh_bkd }}</td>
                        <td style=' border:1px solid #90AFC5 ; color:black'>@currency($valuebk->hrg_bm * $valuebk->jmlh_bkd)</td>
                    </tr>                                    
                @endforeach
                @if($no=="0")                                    
                <tr>
