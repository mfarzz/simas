            ...
            <div class="box">
                ...
                <div class="col-md-12">
                    <div class="box bt-3 border-info">
                      ...
                      <div class="box-body">
                        <p>Nomor Nota : {{ $cek_bkrn->no_bkrn }}</p>
                        <p>Tanggal Nota : {{ $tgl = \Carbon\Carbon::parse($cek_bkrn->tgl_bkrn)->formatLocalized('%d %B %Y'); }}</p>
                        <p>Status : @if($cek_bkrn->status_bkrn==0) Draf @elseif($cek_bkrn->status_bkrn == 1)Kirim @endif</p>
                      </div>
                    </div>
                </div>
                <div class="box-body">    
                ...
                <a href="/barang-keluar-nota-opr-khusus"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar barang keluar nota"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Daftar Nota Barang Keluar (Pemakaian)  </button></a>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id_bkr" id="id_bkr">
...
$('#kategori').change(function(){ 
    if(isTambahData){  
        ...
        if(Kat){
            $.ajax({
            type:"GET",
            url:"/barang-keluar-opr-khusus-item?item="+Kat,
            dataType: 'JSON',
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar-opr-khusus', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (row.status_bkrn == 0) {
                        buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.id_bkr + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-pencil fa-sm"></i></a> ' +
                            '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_bkr + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    }
                    return buttons;
                }
            },
...
function editFunc(id_bkr){  
    isTambahData = false;  
    $.ajax({
        type:"POST",
        url: "{{ url('barang-keluar-opr-khusus-edit') }}",
        data: { id_bkr: id_bkr },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#id_bkr').val(res.id_bkr);            
            $('#kategori').val(res.kd_kt).trigger('change');
            $('#idBarang').val(res.kd_brg).trigger('change');
            $('#id_bkr').val(res.id_bkr);
            $('#jumlah').val(res.jmlh_bkr);            
            
            $.ajax({
                type: "GET",
                url: "/barang-keluar-opr-khusus-item?item=" + res.kd_kt,
                dataType: 'JSON',
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-opr-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
