        ...
        <div class="col-12">                
            <div class="box">
                <div class="box-body ribbon-box">
                    <div class="ribbon ribbon-success">Daftar Barang Keluar Khusus
                    </div>                    
                ...
                <div class="box-body">                  
                    <table class="table table-bordered" id="data-datatable">
                        <x-judul-tabel>
                            ...
                            <x-isi-judul-tabel namakolom="Bidang/UPT/Lembaga" />
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Lembaga/Bidang/UPT</label>
                        <div class="col-sm-8">
                            <select id="idUr" name="id_ur" class="form-select form-control" aria-label="Default select example" >
                                <option value=''>---Silahkan Pilih Lembaga/Bidang/UPT---</option>
                                @foreach ($daftar_unit_rektorat as $baris )
                                    <option value={{ $baris->id }}> {{ $baris->nm_ur }} </option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Nama Penerima</label>
                        <div class="col-sm-8">
                          <input type="text" name="nm_penerima" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Nama Penerima" required>
                        </div>
                    </div>
                    <div class="row mb-4">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-8">
                          <input type="date" name="tgl_keluar" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Tanggal Perolehan" required>
                        </div>
                    </div>
...
$('#kategori').change(function(){    
    ...
    if(kat){
        $.ajax({
           type:"GET",
           url:"/barang-keluar-khusus-subkategori?subKat="+kat,
           dataType: 'JSON',
...
$('#idSubkategori').change(function(){    
    ...
    if(subKat){
        $.ajax({
           type:"GET",
           url:"/barang-keluar-khusus-item?item="+subKat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idItem").empty();
...
function deleteFunc(id){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('barang-keluar-khusus-delete') }}",
            data: { id: id },
            dataType: 'json',
            success: function(res){    
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('barang-keluar-khusus-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
