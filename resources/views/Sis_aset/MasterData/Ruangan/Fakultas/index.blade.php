        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="a_id_arf" id="a_id_arf">
                    <div class="row mb-4">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('ruangan-fakultas-aset') }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'a_kd_arf', name: 'a_kd_arf' },
            { data: 'a_nm_arf', name: 'a_nm_arf' },
            { data: 'a_nip_pj_arf', name: 'a_nip_pj_arf' },
            { data: 'a_nm_pj_arf', name: 'a_nm_pj_arf' },
            { 
                data: 'action',
                name: 'action',
                orderable: false,
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.a_id_arf + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-edit fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" onClick="deleteFunc(' + row.a_id_arf + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a>';
                    return buttons;
                }
            },
        ],
...
function editFunc(a_id_arf){
    $.ajax({
        type:"POST",
        url: "{{ url('ruangan-fakultas-aset-edit') }}",
        data: { a_id_arf: a_id_arf },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#a_id_arf').val(res.a_id_arf);
            $('#kode').val(res.a_kd_arf);
            $('#nama_ruangan').val(res.a_nm_arf);
            $('#nama_pj').val(res.a_nm_pj_arf);
            $('#nip_pj').val(res.a_nip_pj_arf);
        }
    });
}  
 
function deleteFunc(a_id_arf){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var a_id_arf = a_id_arf;
        $.ajax({
            type:"POST",
            url: "{{ url('ruangan-fakultas-aset-delete') }}",
            data: { a_id_arf : a_id_arf },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('ruangan-fakultas-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
