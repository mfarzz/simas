        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">Lokasi Rektorat</h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="javascript: void(0);">Master Data</a></li>
                            <li class="breadcrumb-item"><a href="javascript: void(0);">Lokasi</a></li>
                            <li class="breadcrumb-item active">Rektorat</li>
                        </ol>
                    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h4 class="card-title">Daftar Lokasi Rektorat</h4>
                            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-4">
                        <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Rektorat</label>
                        <div class="col-sm-8">
                            <select id="rektorat" name="rektorat" class="form-select form-control" required >
                                <option value=''>---Silahkan Pilih Rektorat---</option>
                                @foreach ($daftar_rektorat as $baris )
                                    <option value={{ $baris->id_ur }}> {{ $baris->id_ur }} - {{ $baris->nm_ur }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('master-lokasi-rektorat-aset') }}",
        columns: [
...
function editFunc(a_id_al){
    $.ajax({
        type:"POST",
        url: "{{ url('master-lokasi-rektorat-aset-edit') }}",
...
function deleteFunc(a_id_al){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('master-lokasi-rektorat-aset-delete') }}",
            data: { a_id_al : a_id_al },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-lokasi-rektorat-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
