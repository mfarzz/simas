        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">Sub Kategori Barang</h4>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h4 class="card-title">Daftar Sub Kategori Barang</h4>
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="box bt-3 border-info">
                              ...
                              <div class="box-body">
                                <p>Kategori : {{ $data_kategori->a_id_kt }} - {{ $data_kategori->a_nm_kt }}</p>
                              </div>
                            </div>
                        </div>
                        ...
                        <a href="/master-kategori-aset"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar kategori"><button type="button" class="btn btn-success waves-effect waves-light"><i class="fas fa-reply"></i> Kembali Ke Daftar Kategori </button></a>
                    </div>                    
                </div>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="a_id_kt_sub" id="a_id_kt_sub">
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('master-kategori-s-aset', ['encripted_id' => $encripted_id]) }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'a_kd_kt_sub', name: 'a_kd_kt_sub' },
            { data: 'a_no_kt_sub', name: 'a_no_kt_sub' },
            { data: 'a_nm_kt_sub', name: 'a_nm_kt_sub' },
            { 
                data: 'action',
                name: 'action',
                orderable: false,
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += '<a href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-placement="left" title="Ubah Data" onClick="editFunc(' + row.a_id_kt_sub + ')" data-original-title="Edit" class="edit btn btn-warning btn-rounded btn-sm waves-effect"><i class="fa fa-edit fa-sm"></i></a> ' +
                    '<a href="javascript:void(0);" onClick="deleteFunc(' + row.a_id_kt_sub + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> '  +
                    '<a href="/master-kategori-s2-aset/' + row.a_kd_kt_sub_en + '" id="delete-compnay" data-bs-toggle="tooltip" data-bs-placement="left" title="Sub Sub Kategori" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> ';
                    return buttons;
                }
            },
        ],
...
function editFunc(a_id_kt_sub){
    $.ajax({
        type:"POST", 
        url: "{{ url('master-kategori-s-aset-edit') }}",
        data: { a_id_kt_sub : a_id_kt_sub },
        dataType: 'json',
        success: function(res){
            $('.modal-title').html("Form Edit Data");
            $('#data-modal').modal('show');
            $('#a_id_kt_sub').val(res.a_id_kt_sub);
            $('#nama').val(res.a_nm_kt_sub);
        }
    });
}

function deleteFunc(a_id_kt_sub){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var a_id_kt_sub = a_id_kt_sub;
        $.ajax({
            type:"POST",
            url: "{{ url('master-kategori-s-aset-delete') }}",
            data: { a_id_kt_sub : a_id_kt_sub },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('master-kategori-s-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
