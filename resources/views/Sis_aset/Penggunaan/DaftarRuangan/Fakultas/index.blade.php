                        ...
                        <div class="col-sm-8">
                            <select id="idRuangan" name="idRuangan" class="form-select form-control" aria-label="Default select example" required>
                                ...
                                @foreach ($daftar_ruangan as $baris )
                                    <option value={{ $baris->a_id_arf }}> {{ $baris->a_kd_arf }} - {{ $baris->a_nm_arf }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
$('#kategori').change(function(){    
    ...
    if(kat){
        $.ajax({
           type:"GET",
           url:"/daftar-barang-ruangan-opf-aset-subkategori?subKat="+kat,
           dataType: 'JSON',
...
$('#idSubkategori').change(function(){    
    ...
    if(subkat){
        $.ajax({
           type:"GET",
           url:"/daftar-barang-ruangan-opf-aset-sub2kategori?sub2Kat="+subkat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub2kategori').change(function(){    
    ...
    if(sub2kat){
        $.ajax({
           type:"GET",
           url:"/daftar-barang-ruangan-opf-aset-sub3kategori?sub3Kat="+sub2kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub3kategori').change(function(){    
    ...
    if(sub3kat){
        $.ajax({
           type:"GET",
           url:"/daftar-barang-ruangan-opf-aset-barang?sub4Kat="+sub3kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idBarang").empty();
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('daftar-barang-ruangan-opf-aset') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    '<a href="daftar-barang-ruangan-opf-aset-barang-rincian/' + row.a_id_adrf_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian Barang" class="btn btn-rounded btn-sm btn-primary"><i class="fa fa-book fa-sm"></i></a> ';
                    return buttons;
                }
            },
        ],
...
function deleteFunc(a_id_adrf){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('daftar-barang-ruangan-opf-aset-delete') }}",
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('daftar-barang-ruangan-opf-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
