    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Perubahan Koreksi</h5>
            ...
            <div class="modal-body">
                <form action="javascript:void(0)" id="DataForm" name="DataForm" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    ...
                    <fieldset>
                        <legend>Rincian Kapitalisasi</legend>
                        <div class="row mb-4">
                            <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Kuantitas</label>
                            <div class="col-sm-8">
                              <input type="number" id="kuantitas" name="kuantitas" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Kuantitasi" required>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Nilai Aset</label>
                            <div class="col-sm-8">
                              <input type="number" id="nilai_aset" name="nilai_aset" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Nilai Aset" required>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset>
                        ...
                        <div class="row mb-4">
                            <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">No Dasar Koreksi</label>
                            <div class="col-sm-8">
                              <input type="text" id="no_dasar_koreksi" name="no_dasar_koreksi" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Asal Perolehan" required>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <label for="horizontal-firstname-input" class="col-sm-4 col-form-label">Tanggal Dasar Koreksi</label>
                            <div class="col-sm-8">
                              <input type="date" id="tgl_dasar_koreksi" name="tgl_dasar_koreksi" class="form-control" id="horizontal-firstname-input" placeholder="Entrikan Tanggal Dasar Koreksi" required>
                            </div>
                        </div>
                    </fieldset>
...
$('#kategori').change(function(){    
    ...
    if(kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-koreksi-opr-khusus-aset-subkategori?subKat="+kat,
           dataType: 'JSON',
...
$('#idSubkategori').change(function(){    
    ...
    if(subkat){
        $.ajax({
           type:"GET",
           url:"/perubahan-koreksi-opr-khusus-aset-sub2kategori?sub2Kat="+subkat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub2kategori').change(function(){    
    ...
    if(sub2kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-koreksi-opr-khusus-aset-sub3kategori?sub3Kat="+sub2kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub3kategori').change(function(){    
    ...
    if(sub3kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-koreksi-opr-khusus-aset-barang?sub4Kat="+sub3kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idBarang").empty();
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('perubahan-koreksi-opr-khusus-aset') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    buttons += 
                    '<a href="javascript:void(0);" onClick="deleteFunc(' + row.a_id_au + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> '  +
                    '<a href="perubahan-koreksi-opr-khusus-aset-barang/' + row.a_id_au_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian Barang" class="btn btn-rounded btn-sm btn-primary"><i class="fa fa-book fa-sm"></i></a> ';
                    return buttons;
                }
            },
        ],
...
function deleteFunc(a_id_au){
    var pesan = "Apakah anda yakin akan menghapus data ini ?";
    if (confirm(pesan) == true) {
        var a_id_apu= a_id_au;
        $.ajax({
            type:"POST",
            url: "{{ url('perubahan-koreksi-opr-khusus-aset-delete') }}",
            data: { a_id_au : a_id_au },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('perubahan-koreksi-opr-khusus-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
