...
$('#kategori').change(function(){    
    ...
    if(kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-kondisi-opf-khusus-aset-subkategori?subKat="+kat,
           dataType: 'JSON',
...
$('#idSubkategori').change(function(){    
    ...
    if(subkat){
        $.ajax({
           type:"GET",
           url:"/perubahan-kondisi-opf-khusus-aset-sub2kategori?sub2Kat="+subkat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub2kategori').change(function(){    
    ...
    if(sub2kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-kondisi-opf-khusus-aset-sub3kategori?sub3Kat="+sub2kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub3kategori').change(function(){    
    ...
    if(sub3kat){
        $.ajax({
           type:"GET",
           url:"/perubahan-kondisi-opf-khusus-aset-barang?sub4Kat="+sub3kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idBarang").empty();
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('perubahan-kondisi-opf-khusus-aset') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    '<a href="perubahan-kondisi-opf-khusus-aset-barang/' + row.a_id_au_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian Barang" class="btn btn-rounded btn-sm btn-primary"><i class="fa fa-book fa-sm"></i></a> ';
                    return buttons;
                }
            },
        ],
...
function deleteFunc(a_id_au){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('perubahan-kondisi-opf-khusus-aset-delete') }}",
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('perubahan-kondisi-opf-khusus-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
