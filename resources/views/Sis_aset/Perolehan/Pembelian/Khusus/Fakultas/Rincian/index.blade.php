            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="/perolehan-pembelian-opf-khusus-aset"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar pembelian"><button type="button" class="btn btn-success waves-effect waves-light"><i class="fas fa-reply"></i> Kembali Ke Daftar Pembelian </button></a>
                    </div>                    
                </div>
            </div>
        </div>
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('perolehan-pembelian-opf-khusus-aset-rincian', ['encripted_id' => $encripted_id]) }}",
        columns: [
...
function editFunc(a_id_apr){
    $.ajax({
        type:"POST", 
        url: "{{ url('perolehan-pembelian-opf-khusus-aset-rincian-edit') }}",
...
function deleteFunc(a_id_apr){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('perolehan-pembelian-opf-khusus-aset-rincian-delete') }}",
            data: { a_id_apr : a_id_apr },
            dataType: 'json',
            success: function(res){    
                if (res.status === 1) {
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('perolehan-pembelian-opf-khusus-aset-rincian-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
