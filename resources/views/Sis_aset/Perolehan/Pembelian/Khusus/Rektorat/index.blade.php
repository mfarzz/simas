...
$('#kategori').change(function(){    
    ...
    if(kat){
        $.ajax({
           type:"GET",
           url:"/perolehan-pembelian-opr-khusus-aset-subkategori?subKat="+kat,
           dataType: 'JSON',
...
$('#idSubkategori').change(function(){    
    ...
    if(subkat){
        $.ajax({
           type:"GET",
           url:"/perolehan-pembelian-opr-khusus-aset-sub2kategori?sub2Kat="+subkat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub2kategori').change(function(){    
    ...
    if(sub2kat){
        $.ajax({
           type:"GET",
           url:"/perolehan-pembelian-opr-khusus-aset-sub3kategori?sub3Kat="+sub2kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
...
$('#idSub3kategori').change(function(){    
    ...
    if(sub3kat){
        $.ajax({
           type:"GET",
           url:"/perolehan-pembelian-opr-khusus-aset-barang?sub4Kat="+sub3kat,
           dataType: 'JSON',
           success:function(res){               
            if(res){
                $("#idBarang").empty();
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('perolehan-pembelian-opr-khusus-aset') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    if (row.a_tercatat_ap === 3) 
                    {
                        ...
                        '<a href="perolehan-pembelian-opr-khusus-aset-rincian/' + row.a_id_ap_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian SP2D" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> '  +
                        '<a href="perolehan-pembelian-opr-khusus-aset-barang/' + row.a_id_ap_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian Barang" class="btn btn-rounded btn-sm btn-primary"><i class="fa fa-book fa-sm"></i></a> '  +
                        '<a href="perolehan-pembelian-opr-khusus-aset-kib/' + row.a_id_ap_en + '"  data-bs-toggle="tooltip" data-bs-placement="left" title="Rincian KIB" class="btn btn-rounded btn-sm btn-dark"><i class="fa fa-book fa-sm"></i></a> ';
                    }
                    else
                    {
                        buttons += 
...
function deleteFunc(a_id_ap){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('perolehan-pembelian-opr-khusus-aset-delete') }}",
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('perolehan-pembelian-opr-khusus-aset-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
