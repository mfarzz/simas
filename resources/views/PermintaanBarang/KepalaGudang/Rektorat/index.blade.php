...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('permintaan-barang-kg-pimr') }}",
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    buttons +=  
                    '<a href="permintaan-barang-kg-pimr-daftar/' + row.id_pbr_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Daftar Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-book fa-sm"></i></a> '+
                    '<a href="permintaan-barang-kg-pimr-log/' + row.id_pbr_en + '" data-bs-toggle="tooltip" data-bs-placement="left" title="Riwayat Permintaan Barang" class="btn btn-rounded btn-sm btn-success"><i class="fa fa-history fa-sm"></i></a> ';
                    return buttons;
                }
            },     
        ],
...
function validasiFunc(id_pbr){
    ...
    if (confirm(pesan) == true) {
        ...
        $.ajax({
            type:"POST",
            url: "{{ url('permintaan-barang-kg-pimr-validasi') }}",
            data: { id_pbr: id_pbr },
            dataType: 'json',
            success: (data) => {  
                if (data.status === 1) {
