    ...
    <div class="row">
        <div class="col-12">                
            <div class="box">
                <div class="box-body ribbon-box">
                    <div class="ribbon ribbon-success">Daftar Permintaan Barang Item Barang
                        ...
                        <ul class="box-controls pull-right">
                            @if($cek_pbf->untuk_pbs==1 and $cek_pbf->level_pbs==1)
                ...
                <a href="/permintaan-barang-pimf"  data-bs-toggle="tooltip" data-bs-placement="left" title="Kembali ke daftar permintaan barang"><button type="button" class="btn btn-success waves-effect btn-label waves-light"><i class="fa fa-mail-reply"></i> Kembali Ke Daftar Permintaan Barang </button></a>
            </div>
        </div>
    </div>
</section>
...
$('#kategori').change(function(){     
    if(isTambahData){   
        ...
        if(kat){
            $.ajax({
            type:"GET",
            url:"/permintaan-barang-pimf-daftar-item?item="+kat,
            dataType: 'JSON',
...
$(document).ready( function () {
    ...
    $('#data-datatable').DataTable({
        ...
        ajax: "{{ url('permintaan-barang-pimf-daftar', ['encripted_id' => $encripted_id]) }}",
        columns: [
            ...
            {data: 'action',
                ...
                render: function(data, type, row) {
                    var buttons = '';
                    if (row.level_pbs == 1 && row.status_pbfd == 0) {
                    ...
                    '<a href="javascript:void(0);" id="delete-compnay" onClick="deleteFunc(' + row.id_pbfd + ')" data-bs-toggle="tooltip" data-bs-placement="left" title="Hapus Data" title="Hapus Data" class="delete btn btn-danger btn-rounded btn-sm waves-effect"><i class="fa fa-trash fa-sm"></i></a> ';
                    }
                    return buttons;
                }
            },
...
function editFunc(id_pbfd){  
    ...
    $.ajax({
        type:"POST",
        url: "{{ url('permintaan-barang-pimf-daftar-edit') }}",
        ...
        success: function(res){
            ...
            $.ajax({
                type: "GET",
                url: "/permintaan-barang-pimf-daftar-item?item=" + res.kd_kt,
                dataType: 'JSON',
...
$('#DataForm').submit(function(e) {
    ...
    $.ajax({
        type:'POST',
        url: "{{ url('permintaan-barang-pimf-daftar-store')}}",
        data: formData,
        cache:false,
        contentType: false,
        processData: false,
